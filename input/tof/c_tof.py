...
for base_url in base_urls:
	...
	print(num_pages, last_saved_page)
			...
			if page not in dataset.page.unique():
				try:
					with eventlet.Timeout(35):
						print("Page not found in dataset, crawling: ",x, page)
				...
				try:
					...
					if len(title.split("-")) < 2:
						...
						check = 1
						split = s_title.split("-")
						(claim,claim_label) = (split[:-1],split[-1])
					else:
						check = 2
						split = title.split("-")
						(claim,claim_label) = (split[:-1],split[-1])
				...
				date = article.find("span", {"class":"tt-post-date"}).text
				author = article.find("span", {"class":"tt-post-author-name"}).text
